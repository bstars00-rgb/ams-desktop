...
export async function query(page, code) {
  ...
  // Wait specifically for ROWS to appear (genuinely-empty hotels time out -> 0).
  let hasRows = false;
  try {
    await page.waitForFunction(() => {
      ...
      return t && t.querySelectorAll(".el-table__body-wrapper tbody > tr").length > 0;
    }, { timeout: 12000 });
    hasRows = true;
  } catch { /* genuinely empty or slow */ }
  console.log(`[ctrip] query clicked=${clicked}, rows=${hasRows}`);
}
