...
export async function query(page, code) {
  if (!/roomMapping/i.test(page.url())) await page.goto(ROOMMAPPING_URL, { waitUntil: "domcontentloaded" }).catch(() => {});
  const input = page.locator('input[name="hotel-code"]').first();
  await input.waitFor({ timeout: 10000 });
  await input.fill("");
  await input.fill(String(code));
  // Click the "Query" button
  const q = page.locator('button:has-text("Query"), span:has-text("Query")').first();
  await q.click({ timeout: 5000 }).catch(() => {});
  // wait for the Pre-Mapping list to refresh
